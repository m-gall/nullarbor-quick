package Nullarbor::Module::vampr;
use Moo;
...
sub html {
  ...
  my %abr;
  for my $id (@id) {
    my $infile = "$indir/$id/vampr.header.txt";
    $abr{$id} = Nullarbor::Tabular::load(-file=>$infile, -sep=>"\t", -header=>1, -key=>3); # 4 = "GENE"
  }
...
#          );
  ...
  return $self->table_legend("&ge;Present", "Multiple hits", "absent")
