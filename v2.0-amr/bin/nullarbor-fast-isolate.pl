...
my $VERSION = '2.0.20181010-fast hack with amrfinder';
...
#  print $fh "ROARY := roary -v $roary_opt\n";
  ...
  print $fh "AMRFINDER := perl /tmp/nullarbor_mgall/v2.0-amr/plugins/amrfinder/amrfinder.pl -n\n";
  print $fh "VAMPR := perl /tmp/nullarbor_mgall/v2.0-amr/plugins/vampr/VAMP.pl\n";
  print $fh "MLST := mlst\n";
...
report-isolate/index-fast-isolate.html : ref.fa.fai yield mlst core.aln virulome resistome plasmidome amrfinder vamprhead
...
plasmidome : $(addsuffix /plasmidome.tab,$(ISOLATES))

amrfinder : $(addsuffix /amrfinder.tab,$(ISOLATES))

vampr : $(addsuffix /vampr.txt,$(ISOLATES))

vamprnames : $(addsuffix /vampr.names.txt,$(ISOLATES))

vamprkegg : $(addsuffix /vampr.kegg.txt,$(ISOLATES))

vamprjoin : $(addsuffix /vampr.join.kegg.txt,$(ISOLATES))

vamprhead : $(addsuffix /vampr.header.txt,$(ISOLATES))
...
%/amrfinder.tab : %/contigs.fa
  $(AMRFINDER) $^ > $@

%/vampr.txt : %/contigs.fa
  $(VAMPR) $^ > $@

%/vampr.names.txt : %/vampr.txt
  cut -f2 $(@D)/vampr.txt > $(@D)/vampr.names.txt

%/vampr.kegg.txt : %/vampr.names.txt
  cut -f1 -d. $(@D)/vampr.names.txt > $(@D)/vampr.kegg.txt

%/vampr.join.kegg.txt : %/vampr.kegg.txt
  sort $(@D)/vampr.kegg.txt | join -1 1 -2 1 - ${BINDIR}/../plugins/vampr/VAMP_data/KeggNames.txt | awk '{ print 100,100,$$1,$$2 }' > $(@D)/vampr.join.kegg.txt
  sed -i "s|\s|\t|g" $(@D)/vampr.join.kegg.txt
 # rm $(@D)/vampr.names.txt
 # rm  $(@D)/vampr.kegg.txt

%/vampr.header.txt : %/vampr.join.kegg.txt
 echo -e "%IDENTITY\t%COVERAGE\tGENE\tTERM" | cat - $(@D)/vampr.join.kegg.txt > $(@D)/vampr.header.txt
