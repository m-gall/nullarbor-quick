...
$make{'report'} = {
  DEP => [ 'report/index-fast-population.html' ],
};
$make{'report'}{'CMD'} = '$(MAKE) space' unless $keepfiles;

$make{'report/index-fast-population.html'} = {
#  DEP => [ $REF, qw(yields kraken abricate virulome mlst.tab denovo.tab core.aln tree.gif distances.tab roary) ],
#  DEP => [ $REF, qw(yields abricate virulome mlst.tab denovo.tab core.aln distances.tab) ],
  DEP => [ $REF, qw(yields abricate virulome mlst.tab denovo.tab core.aln tree.gif distances.tab) ],
#  CMD => "$FindBin::RealBin/nullarbor-report.pl --name $name --indir $outdir --outdir $outdir/report",
  CMD => "$FindBin::RealBin/nullarbor-report-fast-population.pl --name $name --indir $outdir --outdir $outdir/report",
};
...
#   # $make{$clipped[0]} = {
#   #   DEP => [ @reads ],
#   #   CMD => $trim ? [ "matic PE -threads $CPUS -phred33 @reads $id/$R1 /dev/null $id/$R2 /dev/null ".($cfg->{trimmomatic} || '') ]
...
#  require_exe( qw'trimmomatic prokka roary kraken snippy mlst abricate megahit spades.py nw_order nw_display FastTree snp-dists seqret' );
  require_exe( qw'trimmomatic snippy mlst abricate megahit spades.py nw_order nw_display FastTree snp-dists seqret' );
