...
msg("Nullarbor fast - with roary, kraken and prokka switched off");
...
#  CMD => "$FindBin::RealBin/nullarbor-report.pl --name $name --indir $outdir --outdir $outdir/report",
  CMD => "$FindBin::RealBin/nullarbor-report-fast.pl --name $name --indir $outdir --outdir $outdir/report",
};
...
#  require_exe( qw'trimmomatic prokka roary kraken snippy mlst abricate megahit spades.py nw_order nw_display FastTree snp-dists seqret' );
  require_exe( qw'trimmomatic snippy mlst abricate megahit nw_order nw_display FastTree snp-dists seqret' );
