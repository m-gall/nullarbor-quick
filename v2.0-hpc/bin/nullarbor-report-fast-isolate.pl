...
use FindBin;
#FindBin="/scratch/RDS-FSC-TRG_STM_Val-RW/nullarbor-fast/v2.0/bin";
#use lib "$FindBin::RealBin/../perl5";
use lib "/project/RDS-FMH-PGRepo-RW/tools/nullarbor2.0/v2.0-skippy/perl5";
...
my $TEMPLATE_DIR = "/project/RDS-FMH-PGRepo-RW/tools/nullarbor2.0/v2.0-skippy/conf";
...
my @ids = path("$indir/$name.isolates.txt")->lines({chomp=>1});
...
my @section = qw(jobinfo seqdata mlst resistome virulome plasmidome amrfinder
                  reference
                  tools databases about);
...
for my $section (@section) {
  ...
  load $modname;
  my $module = $modname->new(indir=>$indir, outdir=>$outdir, id=>$section, report=>$report, isolates=>\@ids, name=>$name, samplename=>$samplename);
...
sub usage {
  ...
  print "    --report   Print version and exit\n";
  exit;
}
